...
export const EditProposal = () => {
  ...
  const loadCities = useCallback(async () => {
    const response = await axios.get(
      `https://servicodados.ibge.gov.br/api/v1/localidades/estados/${data['state']}/distritos`
    );
  ...
  useEffect(() => {
    ...
    setValue('date_proposal', data['date_proposal']);
    setValue('cnpj_cpf', data['cnpj_cpf']);
  ...
  }, [data, setValue, loadCities]);

  const sendProposal = handleSubmit(async (data) => {
    try {
      await api.put(`/users/${id}`, data);
      toast.success('Sua proposta foi atualizada com sucesso!');
    } catch (err) {
      toast.error(
